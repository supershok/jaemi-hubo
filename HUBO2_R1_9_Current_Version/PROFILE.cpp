...
// START MARIA'S FUNCTION *******************************************************************
void mariaMotion_seqLRUPDWN(int T, char MotionStop,unsigned int MotionNo)
{
	static int time = 0;
	static int time1 = 0;
	static float ftime = 0.;
...
//	MotionNo = 47;	// user should change
	...
	if(MotionStop == 0)
	{
		...
		else if((time>=0)&&(time<=T))
		{
			...
			FTN_half_1_cos(1.0f,time,    600,100,0,0,&result1[LEB]);
			FTN_half_1_cos(-1.0f,time,900,100,0,0,&result2[LEB]);
